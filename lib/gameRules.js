// ---------------------------------------------------------------------------
// WINGO game rules (from the WINGO Rulebook)
// ---------------------------------------------------------------------------
...
export const NUM_ROWS = 15; // 3 blocks x 5 rows
export const ROWS_PER_BLOCK = 5;
export const NUM_BLOCKS = 3;

// Each column is restricted to a numeric band out of 1-90, standard
// Tambola-style spread, so "ascending order" placement is meaningful.
// Column 0: 1-18, Column 1: 19-36, Column 2: 37-54, Column 3: 55-72, Column 4: 73-90
export function columnBand(colIndex) {
  const size = 18; // 90 / 5
  const start = colIndex * size + 1;
  const end = start + size - 1;
  return [start, end];
}

export function columnForNumber(n) {
  return Math.min(Math.floor((n - 1) / 18), NUM_COLUMNS - 1);
}

// Row roles within a block (0-indexed row within the 5-row block):
// row 0: ODD/EVEN bonus row (only for block 0, i.e. very first row overall)
// row 4: ODD/EVEN bonus row (only for last block, i.e. very last row overall)
// all other rows: ASCENDING bonus rows
// This matches the rulebook screenshot: row 1 = ODD/EVEN, rows 2-4 = ASCENDING,
// rows 6-9 = ASCENDING, rows 11-14 = ASCENDING, row 15 = ODD/EVEN.
export function rowRole(globalRowIndex) {
  if (globalRowIndex === 0) return 'ODD_EVEN';
  if (globalRowIndex === NUM_ROWS - 1) return 'ODD_EVEN';
  return 'ASCENDING';
}
...
export function calculateScore(ticket) {
  ...
  // Complete blocks (all 25 cells in the block filled)
  ...
  // ODD/EVEN bonus rows (first row, last row)
  for (const rowIdx of [0, NUM_ROWS - 1]) {
  ...
  // ASCENDING bonus rows (every row except first/last)
  for (let rowIdx = 1; rowIdx < NUM_ROWS - 1; rowIdx++) {
    const row = ticket[rowIdx];
